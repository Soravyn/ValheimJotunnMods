...
{
    [BepInPlugin("soravyn.CarryWeight", "CarryWeight", "2.0.0")]
    ...
    public class CarryWeight : BaseUnityPlugin
    {
        ...
        void Awake()
        {
            ...
            CWSkill = SkillManager.Instance.AddSkill(new SkillConfig()
            {
                Identifier = "CWSkill",
                Name = "Carry Weight",
        ...
        {
            Texture2D texture = AssetUtils.LoadTexture("CarryWeight/Assets/" + filename);
        ...
        class RaiseSkill_Patch
        {
            static void Postfix(Skills.SkillType skill, ref Player __instance)
            {
                ...
                else if (skill == Skills.SkillType.Swim && raiseSkillCondition(__instance))
                {
